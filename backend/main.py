...
⚠️ 记住：要留的**不只是"用户本人的档案"**。以后还用得上的信息都算 ——
项目的做法与结论、双方约定的称呼、这台机器/这个环境里的规矩，都该记下来。
...
· **值得复用的结论**（看着不像"用户信息"，但以后真会用到）：
  项目里的技术选型、踩过的坑、验证过的参数、环境约定。
  例：`长期|Ollama 地址必须用 127.0.0.1，写 localhost 会走 IPv6 慢两千倍`
...
· 助手自己的客套话与过程叙述（"好的""明白""我来帮你看看"）。
  ⚠️ 但**对话里得出的结论、约定、命名要记**（见上面那两类）——
  别因为"这是助手说的"就把它们一起丢掉。
· 搜索结果、知识库资料**本身**（那是资料，不是"你该记住的事"）
...
_TEXT_TOOL_DOCS = {
    ...
    "map_plan": '【地图】查地点 / 规划路线，并把结果画成地图卡片给用户看。'
                ...
                'mode 可选 driving(默认)/foot/bike，但**免费路网只提供驾车路径**：'
                '给步行/骑行时距离仍是驾车路线的、时间按速度估算，返回里带 note，'
                '**必须把这话转告用户**，不能让他以为那是真的步行路线。'
                ...
                '⚠️ 这个工具**没有任何实时路况数据** —— 不许说「避开拥堵」「当前畅通」这类话。'
    ...
    "nearby_places": '【地图】查某个地点**周围**有哪些场所（按半径+类别），并在图上标出来。'
                     ...
                     'place 也接受 "23.35,116.68"；'
                     'category 用中文日常说法即可（餐厅/咖啡馆/便利店/超市/药店/医院/'
                     '银行/加油站/停车场/酒店/学校/公交站/公园/厕所）；radius 单位米，默认 1500。'
                     '⚠️⚠️ **本工具没有任何评分数据** —— OSM 不提供评分/星级/评论，'
                     '返回里也没有分数。**绝不许自己编评分、星级或评价**'
                     '（「评分 4.5」「口碑很好」「味道不错」这类全是编的）；'
                     '每条只带一句「这条记录全不全」的说明，照实转述即可。'
                     '⚠️ 中国的小微店铺在 OSM 上覆盖稀疏，**查不到不等于没有**，'
                     '如实说明，并建议换类别词或扩大半径再试。'
...
class _SystemPrompt:
    ...
    def build(last_user_text: str, session: str = "", docs: list = None,
        ...
        parts = [
            ...
            "- 地图：用户说「怎么走 / 规划路线 / 从A到B多远多久 / 某地方在哪 / 在地图上标出来」"
            "时调用 map_plan，它会画一张地图卡片直接发给用户看。"
            "返回里是**真实**距离与用时，照实念，不要自己算。\n"
            + (
                (
