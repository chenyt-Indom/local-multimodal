...
_TEXT_TOOL_DOCS = {
    ...
    "ask_user": '信息不足时**在界面上弹问答框**问用户（**别在回答里用文字问**）。'
                ...
                '"options": ["候选答案1", "候选答案2"]}]}，最多 4 个问题。'
                '⚠️ 一次问清、只问一次；**用户已经说清楚了就别问，直接做**。',
...
async def chat(req: ChatRequest):
    ...
    sys_prompt += (
        ...
        "**先调用 ask_user 问 2~4 个最关键的问题**，拿到答复再动手。"
        "（要弹问答框，**不要在回答里用文字问**。）\n"
        ...
        "解释概念、算个数；或者关键信息你自己查得到（search_knowledge / web_search）。"
        "**别为了显得周到而问 —— 信息够就直接干。**\n"
        ...
        if code_model_on:
            ...
            if code_text_tools:
                ...
                full_sys += (
                    ...
                    "⚠️ **一轮问清、只问一次**。用户已经说清楚了（或只是让你改一行、跑一下、"
                    "解释一段代码）→ **别问，直接做**。\n\n"
                    "【做一整个项目时，按这个顺序自动推进】\n"
